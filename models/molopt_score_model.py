...
class AttentionPooling(nn.Module):
    def __init__(self, input_dim):
        ...
        self.attention_weights = nn.Parameter(torch.randn(input_dim*4))  # shape [input_dim] (128)
        self.liner1 = nn.Linear(input_dim, input_dim*4)
        self.sig = nn.Sigmoid()
    def forward(self, A):
        ...
        A = self.liner1(A)
        A = self.sig(A)
        attention_scores = torch.matmul(A, self.attention_weights)  # shape [num_atom]
        attention_weights = F.softmax(attention_scores, dim=0)  # shape [num_atom]
        weighted_rep = torch.sum(A * attention_weights.unsqueeze(1), dim=0)  # shape [input_dim]
        return weighted_rep
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        self.step_weight = self.compute_all_weights()

        # self.step_weight = self.compute_line_all_weights()


    def get_output_embedding_score(self, mapped_embeddings, inputs):
        # print(mapped_embeddings.shape, inputs.shape)
        align_loss = 1 - torch.nn.functional.cosine_similarity(
            mapped_embeddings.unsqueeze(0), inputs.unsqueeze(0), dim=-1, eps=1e-8
        )
        return align_loss  # Align loss
    

    def compute_all_weights(self, max_step = 1000, target_weight = 0.9, mid_point = 300):
        k = 0.01  # 这个参数控制函数陡峭程度  
        
        weights = torch.zeros(self.config.num_diffusion_timesteps)
        
        for i in range(self.config.num_diffusion_timesteps):
            weights[i] = 2 * (1 / (1 + np.exp(-k * (i - self.config.num_diffusion_timesteps/2))))  
         
        #[0, 2] --> [0, 1]  
        weights = weights / 2  # 现在范围为 [0, 1]  

        #[0, 1] --> [0.8, 1.2]  
        weights = 0.8 + 0.4 * weights  # 线性变换到 [0.8, 1.2]
        
        step_weight = torch.flip(weights, [0])
     
        # with open("weight+0.8_1.2.csv", "w") as f:
        #     for i in range(len(step_weight)):
        #         f.write(str(i) + "," + str(step_weight[i].item()) + "\n")
        
        return torch.flip(weights, [0]) 


    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
                time_step=None, return_all=False, fix_x=False, embedding_feature=None):
        ...
        align_score = torch.zeros(batch_size).to(final_ligand_v)
        for i in range(batch_size):

            if isinstance(embedding_feature[i], torch.Tensor):
                # [num_atom,128]
                ligand_h_i = final_ligand_h[batch_ligand == i]
                
                rep_ligand_h_i = self.attention_pooling(ligand_h_i)
                align_score_i = self.get_output_embedding_score(rep_ligand_h_i, embedding_feature[i])

                align_score[i] = align_score_i * self.step_weight[time_step[i]]
    ...
    def get_diffusion_loss(
            self, protein_pos, protein_v, affinity, batch_protein, ligand_pos, ligand_v, batch_ligand, time_step=None, embedding_feature=None, 
    ):
        ...
        if self.use_classifier_guide:
            ...
            loss = loss_pos + loss_v * self.loss_v_weight + loss_exp * self.loss_exp_weight + 0.1* torch.mean(preds['align_score'] )
            # quit()
